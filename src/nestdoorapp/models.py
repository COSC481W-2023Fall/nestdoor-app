...
class Reply(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(check=models.Q(
                content__isnull=False), name="reply_content_missing"),
            ...
            # 'post_moderation_content_missing': 'CHECK (moderated_note IS NOT NULL)',
            models.CheckConstraint(check=models.Q(
                reply_id__isnull=False), name="reply_id_missing")
...
class Building(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(
                address__isnull=False), name="building_address_missing"),
            # 'building_address_missing': 'CHECK (address IS NOT NULL)',
            models.CheckConstraint(check=models.Q(
                description__isnull=False), name="building_description_missing")
            # 'building_description_missing': 'CHECK (description IS NOT NULL)'
        ]


# many to many linking table for when posts mention buildings
class BuildingPost(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(
                id__isnull=False), name="buildingpost_id_missing")
            # 'id_missing': 'CHECK (id IS NOT NULL)',
        ]


# many to many linking table for when replies mention buildings
class BuildingReply(models.Model):
    id = models.AutoField(primary_key=True)
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(
                id__isnull=False), name="buildingreply_id_missing")
            # 'id_missing': 'CHECK (id IS NOT NULL)',
        ]
    ...
    def clean(self):
        ...
        if self.building_id is None and (self.gps_longitude is None or self.gps_latitude is None):
            raise ValidationError(
                'Either the building or gps coordinates must be specified.')
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(building_id__isnull=False) |
                                   (models.Q(gps_longitude__isnull=False) &
                                    (models.Q(gps_latitude__isnull=False))),
            ...
            models.CheckConstraint(check=models.Q(
                description__isnull=False), name="location_description_missing"),
            # 'location_description_missing': 'CHECK (description IS NOT NULL)',
            models.CheckConstraint(check=models.Q(
                location_id__isnull=False), name="location_id_missing")
            # 'location_id_missing': 'CHECK (location_id IS NOT NULL)',
        ]


# many to many linking table for when posts mention locations
class LocationPost(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(
                id__isnull=False), name="locationpost_id_missing")
            # 'id_missing': 'CHECK (id IS NOT NULL)',
        ]


# many to many linking table for when replies mention locations
class LocationReply(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(check=models.Q(
                id__isnull=False), name="locationreply_id_missing")
            # 'id_missing': 'CHECK (id IS NOT NULL)',
        ]
